...
export default function Lucky777Page() {
  ...
  const mutation = useMutation({
    mutationFn: (payload: Lucky777Payload) =>
      api.post<FormSubmittedDto>('/api/forms/777', payload),
    onSuccess: () => {
      setShowSuccess(true)
  ...
  const handleSubmit = () => {
    if (!rows.every((r) => r.length === PICK_COUNT)) {
      setValidationError(t('lucky777.validation'))
      return
    }
    ...
    <div className="max-w-3xl mx-auto space-y-4 pb-8">
        ...
        <div key={rowIdx} className="card p-4 space-y-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-2">
              {t('lucky777.numbers')} ({row.length}/{PICK_COUNT})
            </p>
      ...
      <div className="card p-4 space-y-2">
        <div className="flex justify-between text-sm">
          <span className="text-gray-500">{t('lucky777.costPerRow')}</span>
          <span className="font-semibold">{t('common.currency')}{COST_PER_ROW.toFixed(2)}</span>
        </div>
        <div className="flex justify-between text-base font-bold">
          {/* Fix 3: was t('lotto.totalCost') → correct key */}
          <span>{t('lucky777.totalCost')}</span>
